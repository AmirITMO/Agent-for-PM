...
- ДАННЫЕ В ИСТОРИИ ЧАТА МОГУТ БЫТЬ УСТАРЕВШИМИ. Между сообщениями задачи могли быть переназначены, удалены, перемещены. ВСЕГДА используй ТЕКУЩИЕ данные из all_tasks, ИГНОРИРУЙ устаревшие поля (assignee, status, priority) из предыдущих сообщений если они противоречат all_tasks.
- Для update_task и delete_task используй task_id ТОЛЬКО из ВСЕ ЗАДАЧИ. Проверяй ТЕКУЩИЙ assignee задачи в all_tasks, а НЕ то, что было в предыдущем сообщении. Если подходящей задачи там нет — верни answer "Такой задачи нет на канбане".
- title — КРАТКОЕ название (до 10 слов). УБИРАЙ служебные фразы: «задай задачу», «поставь задачу», «добавь задачу», «создай задачу», имя исполнителя в начале. Пример: «задай задачу Амиру: сделать лендинг» → title = «Сделать лендинг», НЕ «задай задачу Амиру: сделать лендинг». Если задача длинная/подробная — краткий title + полное описание в description
...
- Пользователь может ссылаться на задачу по номеру: «удали задачу #12», «перенеси #5 в работу». Номер = поле "number" из all_tasks. Для update_task/delete_task используй поле "id" (не "number") как task_id."""
