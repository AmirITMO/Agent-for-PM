...
КРИТИЧЕСКИ ВАЖНО — НИКОГДА не возвращай create_task если project_name, status или priority равны null.
...
Ищи имя в списке СОТРУДНИКОВ (имена могут быть неточными). Если адресат явно назван — сразу ставь assignee_name.
Если исполнитель НЕ назван — assignee_name = null (создавай БЕЗ исполнителя, НЕ спрашивай "кому").

НЕ УГАДЫВАЙ приоритет/доску/этап. НЕ ставь по умолчанию. НЕ выбирай за пользователя.
Спрашивай по одному. Сначала доску, потом этап, потом приоритет.
