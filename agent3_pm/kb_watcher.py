...
PARSE_PROMPT = """Из текста созвона/записи извлеки ВСЕ задачи которые были упомянуты.
...
async def _parse_tasks_from_content(content: str, filename: str) -> list[dict]:
...
async def check_kb_updates(bot):
    ...
    for folder in KB_FOLDERS:
        ...
        for f in new_files:
            ...
            tasks = await _parse_tasks_from_content(content, f["name"])
            if not tasks:
                logger.info(f"No tasks found in {filepath}")
                continue

            # Mark bugs from bugs/ folder
            if "bugs" in folder:
                for t in tasks:
                    t["is_bug"] = True
