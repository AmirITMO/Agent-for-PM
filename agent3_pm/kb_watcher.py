...
async def _parse_tasks_from_content(content: str, filename: str,
    ...
    try:
        ...
        logger.info(f"GPT response for {filename}: {raw[:500]}")
        result = json.loads(raw)

        def _extract_list(obj, depth=0):
            if depth > 3:
                return []
            if isinstance(obj, list):
                return obj
            if isinstance(obj, dict):
                for key in ("result", "tasks", "bugs", "items", "data"):
                    if key in obj:
                        found = _extract_list(obj[key], depth + 1)
                        if found:
                            return found
                for v in obj.values():
        ...
        tasks = _extract_list(result)
        if not tasks:
            logger.warning(f"GPT returned no extractable tasks: {raw[:200]}")
        return tasks
    except Exception:
